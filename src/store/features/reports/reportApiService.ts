...
export const reportsApiService = baseApi.injectEndpoints({
  endpoints: (builder) => ({
    ...
    // GET /api/reports/finance/product-wise-profit-loss
    getProductWiseProfitLoss: builder.query<
      ReportResponse,
      { start_date?: string; end_date?: string }
    >({
      query: (params) => ({
        url: "/reports/finance/product-wise-profit-loss",
        method: "GET",
        params,
      }),
      providesTags: ["Reports"],
...
export const {
  ...
  useGetProductWiseProfitLossQuery,
} = reportsApiService;
